...
def path_loss(d_jn, fc_ghz):
    # Eq (2): Path loss in dB between RU j and UE n.
    # d_jn in meters and fc_ghz in GHz.
    return 28 + 22 * np.log10(d_jn) + 20 * np.log10(fc_ghz)


def free_space_path_loss(d_jn, fc_ghz):
    # Eq (3) : Free space path loss in dB between RU j and UE n.
    # d_jn in meters and fc_ghz in GHz.
    return 32.45 + 20 * np.log10(d_jn) + 20 * np.log10(fc_ghz)

def channel_coefficient(antenna_gain, path_loss_db, rician_factor_k):
    # Eq (4) : Channel coefficient between RU j and UE n.
    path_loss_linear  = 10 ** (path_loss_db / 10)
    # w_jn captures small scale rician fading.
    w_jn  = np.sqrt(rician_factor_k / (rician_factor_k + 1)) + \
